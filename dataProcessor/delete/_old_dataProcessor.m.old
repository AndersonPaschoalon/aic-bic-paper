...
%prepare interArrival
interArrival = abs(sort(M));
interArrival(interArrival == 0) =  min_time;
interArrival = interArrival*scale;

% ESTIMATION DATASET -- all models will be evaluated with this dataset
interArrival1 = interArrival(1:2:end);

% CROSS-VALIDATION DATASET -- all the movel validation (aic, bic, hurst, avarage, 
% qq, ...) will be done with this dataset
interArrival2 = interArrival(2:2:end);
[interArrival1 interArrival2] = sameLength(interArrival1, interArrival2)


%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Run
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
max_time = max(interArrival1);
interArrivalCdf = empiricalCdf(interArrival1);
m = length(interArrival1);

%plot empirical Cumulative distribution function
fprintf("Ploting inter-arrival cumulative distribution\n");
plotData(interArrival1, interArrivalCdf, 'interArrival', 'F(interArrival)', 'b-', 'Inter-arrival empirical CDF');
saveas(gca , 'figures/Inter-arrival empirical CDF.png');
title = "Empirical CDF function";
labels = "interArrival1(x), interArrivalCdf(y)";
filename = strcat(PLOT_DIR, title, PLOT_DATA_EXT);
matrix2File([interArrival1 interArrivalCdf], filename, title, labels);
...
if(NORMAL_FITTING != 0)
    fprintf("\n*********************\nNormal data fitting\n*********************\n");
    normal_mean = mean(interArrival1);
    normal_std = std(interArrival1);
    fprintf("Mean = %f \n", normal_mean);
    fprintf("Std = %f \n", normal_std);
...
if(PARETO_FITTING != 0)
    ...
    pareto_mlh_xm = min(interArrival1);
	pareto_mlh_alpha_div = sum(log(interArrival1) - log(pareto_mlh_xm));
	if( abs(real(pareto_mlh_alpha_div)) < INFINITEZIMAL) 
		pareto_mlh_alpha_div = INFINITEZIMAL;
	endif
        pareto_mlh_alpha = length(interArrival1)/(pareto_mlh_alpha_div);
...
% Calc IAC and BIC
if (WEIBULL_FITTING != 0)
	weibull_bic = informationCriterion(interArrival2, 'weibull', [weibull_alpha weibull_betha], 'bic');
	weibull_aic = informationCriterion(interArrival2, 'weibull', [weibull_alpha weibull_betha], 'aic');
endif

if (NORMAL_FITTING != 0)
	normal_bic = informationCriterion(interArrival2, 'normal', [normal_mean normal_std], 'bic');
	normal_aic = informationCriterion(interArrival2, 'normal', [normal_mean normal_std], 'aic');
endif

if (EXPONENTIAL_FITTING != 0)	
	exponential_lr_bic = informationCriterion(interArrival2, 'exponential', [exp_lambda], 'bic');
	exponential_lr_aic = informationCriterion(interArrival2, 'exponential', [exp_lambda], 'aic');
	exponential_me_bic = informationCriterion(interArrival2, 'exponential', [exp_lambdaMean], 'bic');
	exponential_me_aic = informationCriterion(interArrival2, 'exponential', [exp_lambdaMean], 'aic');
endif

if (PARETO_FITTING != 0)
	pareto_lr_bic = informationCriterion(interArrival2, 'pareto', [pareto_alpha pareto_xm], 'bic');
	pareto_lr_aic = informationCriterion(interArrival2, 'pareto', [pareto_alpha pareto_xm], 'aic');
	pareto_mlh_bic = informationCriterion(interArrival2, 'pareto', [pareto_mlh_alpha pareto_mlh_xm], 'bic');
	pareto_mlh_aic = informationCriterion(interArrival2, 'pareto', [pareto_mlh_alpha pareto_mlh_xm], 'aic');
endif

if(CAUCHY_FITTING != 0)	
	cauchy_bic = informationCriterion(interArrival2, 'cauchy', [cauchy_gamma cauchy_x0], 'bic');
	cauchy_aic = informationCriterion(interArrival2, 'cauchy', [cauchy_gamma cauchy_x0], 'aic');
endif
...
if(WEIBULL_FITTING != 0)
	figure; %new figure
	%plotData( interArrival2, weibull_interArrival, 'empirical', 'weibull', '-g', 'QQplot - Weibull');
	qqxy = qqPlot( interArrival2, weibull_interArrival, 'empirical', 'weibull', '-g', 'QQplot - Weibull');
...
if(NORMAL_FITTING != 0)
	figure; %new figure
	%plotData(interArrival2, normal_interArrival, 'empirical', 'normal', '-g', 'QQplot - Normal');
	qqxy = qqPlot(interArrival2, normal_interArrival, 'empirical', 'normal', '-g', 'QQplot - Normal');
...
if(EXPONENTIAL_FITTING != 0)
	figure; %new figure
	%plotData(interArrival2, exponential_lr_interArrival, 'empirical', 'exponential (Linear Regression)', '-g', 'QQplot - Exponential (Linear Regression)');
	qqxy = qqPlot(interArrival2, exponential_lr_interArrival, 'empirical', 'exponential (Linear Regression)', '-g', 'QQplot - Exponential (Linear Regression)');
	...
	figure; %new figure
	%plotData(interArrival2, exponential_me_interArrival, 'empirical', 'exponential (Mean)', '-g', 'QQplot - Exponential (Mean)');
	qqxy = qqPlot(interArrival2, exponential_me_interArrival, 'empirical', 'exponential (Mean)', '-g', 'QQplot - Exponential (Mean)');
...
if(PARETO_FITTING != 0)
	figure; %new figure
	%plotData(interArrival2, pareto_lr_interArrival, 'empirical', 'pareto (Linear Regression)', '-g', 'QQplot - Pareto (Linear Regression)');
	qqxy = qqPlot(interArrival2, pareto_lr_interArrival, 'empirical', 'pareto (Linear Regression)', '-g', 'QQplot - Pareto (Linear Regression)');
	...
	figure; %new figure
	%plotData(interArrival2, pareto_mlh_interArrival, 'empirical', 'pareto (Maximum Likehood)', '-g', 'QQplot - Pareto (Maximum Likehood)');
	qqxy = qqPlot(interArrival2, pareto_mlh_interArrival, 'empirical', 'pareto (Maximum Likehood)', '-g', 'QQplot - Pareto (Maximum Likehood)');
...
if(CAUCHY_FITTING != 0)
	figure; %new figure
	%plotData(interArrival2, cauchy_interArrival, 'empirical', 'Cauchy', '-g', 'QQplot - Cauchy');
	qqxy = qqPlot(interArrival2, cauchy_interArrival, 'empirical', 'Cauchy', '-g', 'QQplot - Cauchy');
...
fprintf('+----------------+--------------+---------------+\n');
fprintf('|Original        |%f\t|%f\t|\n',mean(interArrival2), std(interArrival2));
if(WEIBULL_FITTING != 0)
...
M = zeros(4, 1);
M(1) = 0;
M(2) = mean(interArrival2);
M(3) = std(interArrival2);
M(4) = hurst(interArrival2);
...
for i = 1:EVAL_REPETITIONS
	...
	if(WEIBULL_FITTING != 0)
		%compute correlation
		M_w(1, i) = corr(interArrival2, weibull_interArrival);
	...
	if(NORMAL_FITTING != 0)
		M_n(1, i) = corr(interArrival2, normal_interArrival);
	...
	if(EXPONENTIAL_FITTING != 0)
		M_elr(1, i) = corr(interArrival2, exponential_lr_interArrival);
		...
		M_eme(1, i) = corr(interArrival2, exponential_me_interArrival);
	...
	if(CAUCHY_FITTING != 0)
		M_c(1, i) = corr(interArrival2, cauchy_interArrival);
